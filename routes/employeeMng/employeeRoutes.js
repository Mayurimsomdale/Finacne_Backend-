    ...
    // ✅ UPDATED: uan_number added to EMP_SELECT, POST (INSERT), PUT (UPDATE)
...
async function generateEmployeeId(client) {
  ...
  return `${prefix}${String(nextSeq).padStart(4, '0')}`;
    ...
    // ── Full SELECT — ✅ includes uan_number ──────────────────────────────────────
    const EMP_SELECT = `
      SELECT
        e.id, e.employee_id,
    ...
    function buildSnapshot(emp) {
      return {
        ...
        uan_number: emp.uan_number,                                         // ✅ included in snapshot
    ...
    async function restoreFromSnapshot(client, empId, snapshot, newStatus = 'inactive') {
      ...
      `, [
        ...
        s.uan_number || null,                                               // ✅ $17
        s.family_member_name, s.family_contact_no,
    ...
    router.get('/export/template', async (req, res) => {
      try {
        ...
        const sections = [
          { label: 'Personal Information', start: 1,  end: 13, color: 'FF1D4ED8' }, // +1 for UAN
        ...
        const columns = [
          ...
          { header: 'UAN Number',          key: 'uan_number',          width: 14, req: false }, // ✅ NEW
    ...
    router.get('/export/data', async (req, res) => {
      try {
        ...
        const headers = [
          ...
          'PAN Number','Aadhaar Number','UAN Number',                            // ✅ UAN added
    ...
    router.post('/:id/upload-photo', (req, res, next) => {
      ...
      try {
        ...
        await client.query('COMMIT');
        return res.json({ success: true, message: 'Photo uploaded and saved successfully', data: rows[0] });
      } catch (err) {
        await client.query('ROLLBACK');
        try { fs.unlinkSync(req.file.path); } catch (_) {}
    ...
    router.post('/:id/send-rejoin-invite', async (req, res) => {
      ...
      try {
        ...
        await client.query('BEGIN');
      // When inserting into registration_links for a rejoin:
  await client.query(
    ...
    router.post('/', async (req, res) => {
      ...
      try {
        ...
        // ── Validate UAN number if provided ──────────────────────────────────────
        const uanNumber = b.uanNumber?.toString().trim() || null;
        if (uanNumber && !/^\d{1,12}$/.test(uanNumber)) {
          cleanupFiles(req.files);
          await client.query('ROLLBACK');
          return res.status(400).json({ success: false, message: 'UAN Number must be numeric and up to 12 digits.' });
        }
