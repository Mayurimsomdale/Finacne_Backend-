...
function buildCommonFields(d) {
  ...
  return [
    str(d.firstName), str(d.fatherHusbandName), str(d.lastName),                    // 1-3
    str(d.email)?.toLowerCase(),                                                      // 4
    str(d.phone), str(d.altPhone),                                                    // 5-6
    dateStr(d.dob), str(d.gender), str(d.maritalStatus),                             // 7-9
    str(d.educationalQualification), str(d.bloodGroup),                              // 10-11
    str(d.panNumber)?.toUpperCase() || null, str(d.nameOnPan),                       // 12-13
    str(d.aadhar)?.replace(/\s/g, '') || null, str(d.nameOnAadhar),                 // 14-15
    strUan(d.uanNumber),                                                              // 16
    str(d.familyMemberName), str(d.familyContactNo), str(d.familyWorkingStatus),    // 17-19
    str(d.familyEmployerName), str(d.familyEmployerContact),                         // 20-21
    str(d.emergencyContactName), str(d.emergencyContactNo),                          // 22-23
    str(d.emergencyContactAddress), str(d.emergencyContactRelation),                 // 24-25
    str(d.permanentAddress), str(d.permanentPhone),                                   // 26-27
    str(d.permanentLandmark), str(d.permanentLatLong),                               // 28-29
    ...
    str(d.ref1Name), str(d.ref1Designation), str(d.ref1Organization),               // 35-37
    str(d.ref1Address), str(d.ref1CityStatePin),                                     // 38-39
    str(d.ref1ContactNo), str(d.ref1Email),                                          // 40-41
    str(d.ref2Name), str(d.ref2Designation), str(d.ref2Organization),               // 42-44
    str(d.ref2Address), str(d.ref2CityStatePin),                                     // 45-46
    str(d.ref2ContactNo), str(d.ref2Email),                                          // 47-48
    str(d.ref3Name), str(d.ref3Designation), str(d.ref3Organization),               // 49-51
    str(d.ref3Address), str(d.ref3CityStatePin),                                     // 52-53
    str(d.ref3ContactNo), str(d.ref3Email),                                          // 54-55
    str(d.department), str(d.position),                                              // 56-57
    str(d.circle), str(d.projectName),                                               // 58-59
    dateStr(d.joiningDate), str(d.reportingManager), str(d.employmentType),          // 60-62
    str(d.bankName), str(d.accountNumber),                                            // 63-64
    str(d.ifscCode)?.toUpperCase() || null,                                          // 65
    str(d.accountHolderName), str(d.bankBranch),                                     // 66-67
  ]; // length 67
}
...
async function replaceDocuments(client, empDbId, files) {
  ...
  await client.query('DELETE FROM employee_documents WHERE employee_id = $1', [empDbId]);
  await Promise.all(
...
function buildSnapshot(emp) {
  return {
    ...
    father_husband_name: emp.father_husband_name, email: emp.email, phone: emp.phone,
    alt_phone: emp.alt_phone, date_of_birth: emp.date_of_birth, gender: emp.gender,
    marital_status: emp.marital_status, educational_qualification: emp.educational_qualification,
    blood_group: emp.blood_group, pan_number: emp.pan_number, name_on_pan: emp.name_on_pan,
    aadhar_number: emp.aadhar_number, name_on_aadhar: emp.name_on_aadhar,
    uan_number: emp.uan_number,
    family_member_name: emp.family_member_name, family_contact_no: emp.family_contact_no,
    family_working_status: emp.family_working_status, family_employer_name: emp.family_employer_name,
    ...
    local_phone: emp.local_phone, local_landmark: emp.local_landmark,
    ...
    ref1_city_state_pin: emp.ref1_city_state_pin, ref1_contact_no: emp.ref1_contact_no,
    ...
    ref2_city_state_pin: emp.ref2_city_state_pin, ref2_contact_no: emp.ref2_contact_no,
    ...
    ref3_city_state_pin: emp.ref3_city_state_pin, ref3_contact_no: emp.ref3_contact_no,
...
export const submitRegistration = async (req, res) => {
  ...
  try {
    ...
    // ── New employee ──────────────────────────────────────────────────────
      ...
      `, [
        registrationLink.id,
        cf[0],cf[1],cf[2],cf[3],cf[4],cf[5],cf[6],cf[7],cf[8],
        cf[9],cf[10],cf[11],cf[12],cf[13],cf[14],cf[15],
        cf[16],cf[17],cf[18],cf[19],cf[20],cf[21],cf[22],cf[23],cf[24],
        cf[25],cf[26],cf[27],cf[28],cf[29],cf[30],cf[31],cf[32],cf[33],
        cf[34],cf[35],cf[36],cf[37],cf[38],cf[39],cf[40],
        cf[41],cf[42],cf[43],cf[44],cf[45],cf[46],cf[47],
        cf[48],cf[49],cf[50],cf[51],cf[52],cf[53],cf[54],
        cf[55],cf[56],cf[57],cf[58],cf[59],cf[60],cf[61],
        cf[62],cf[63],cf[64],cf[65],cf[66],
    ...
    const flowLabel = submissionType === 'rejoin' ? 're-join' : submissionType === 'resubmit' ? 're' : '';
    ...
    setImmediate(async () => {
      if (email) {
        await sendFormSubmissionConfirmation({
          to: email, formData: formDataForEmail, isRejoin: submissionType === 'rejoin',
        }).catch((e) => console.error('Confirmation email failed:', e.message));
      }
      await sendHRSubmissionNotification({
        formData: formDataForEmail, employeeDbId, isRejoin: submissionType === 'rejoin',
      }).catch((e) => console.error('HR notification email failed:', e.message));
    });

    const messages = {
      rejoin:    'Rejoin request submitted successfully. HR will review your application.',
      resubmit:  'Registration resubmitted successfully. HR will review your updated application.',
...
export const approve = async (req, res) => {
  ...
  try {
    ...
    // Generate KYE PDF asynchronously
    let pdfBase64 = null;
    try {
    ...
    setImmediate(async () => {
      try {
        await sendApprovalEmailWithKYEPdf({
          to: employee.email, firstName: employee.first_name,
          lastName: employee.last_name, employeeId: newEmployeeId,
          isRejoin, pdfBase64, uploadUrl,
      ...
      try {
        await sendHRApprovalNotification({
          ...
          email: employee.email, department: employee.department, isRejoin,
        });
      } catch (e) {
...
export const rejectRejoin = async (req, res) => {
  ...
  try {
    ...
    // Create a fresh rejoin re-edit link
    ...
    console.log(`↩️  [rejectRejoin] Employee ${emp.id} — snapshot restored, re-edit link: ${newLinkId}`);
      ...
      try {
        if (emp.email) {
          await sendRejoinDeclinedEmail({
            to: emp.email, firstName: emp.first_name, lastName: emp.last_name,
            employeeId: emp.employee_id, reason: reason || '',
            rejoinUrl, rejoinUrlExpiry: expiresAt.toISOString(),
          });
        }
      } catch (e) {
    ...
    return res.json({
      success: true,
      message: 'Rejoin request declined — employee data restored. A re-edit link has been sent.',
    });
  } catch (err) {
