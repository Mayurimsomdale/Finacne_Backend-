...
// All business logic, DB queries and file operations live here.
// Each export maps 1-to-1 with a route defined in employeeRoutes.js.
...
import { PROJECT_ROOT, cleanupFiles } from "../../middleware/employeeMng/employeeMiddleware.js"
...
/**
 * Generates the next employee ID in format Insta-YYMMxxxx (e.g. Insta-26050001).
 * Sequence resets per month, padded to 4 digits.
 */
...
/**
 * Inserts a row into employee_documents.
 */
async function saveDocument(client, empDbId, type, file) {
  ...
  await client.query(
    ...
    [
      empDbId,
      type,
      `/uploads/employee_docs/${file.filename}`,
      file.originalname,
      file.size,
      file.mimetype,
    ]
...
/** Formats a date value to YYYY-MM-DD or '' */
const fmtDate = (v) => {
  if (!v) return '';
  const d = new Date(v);
...
/**
 * Writes a row to employee_status_history.
 */
...
/**
 * Builds a plain snapshot object of all mutable employee fields.
 * Stored in employees.rejoin_snapshot before sending a rejoin invite.
 */
function buildSnapshot(emp) {
  return {
    ...
    pan_number: emp.pan_number, name_on_pan: emp.name_on_pan,
    aadhar_number: emp.aadhar_number, name_on_aadhar: emp.name_on_aadhar,
    ...
    local_address: emp.local_address, local_phone: emp.local_phone,
    local_landmark: emp.local_landmark, local_lat_long: emp.local_lat_long,
    ref1_name: emp.ref1_name, ref1_designation: emp.ref1_designation,
    ref1_organization: emp.ref1_organization, ref1_address: emp.ref1_address,
    ref1_city_state_pin: emp.ref1_city_state_pin,
    ref1_contact_no: emp.ref1_contact_no, ref1_email: emp.ref1_email,
    ref2_name: emp.ref2_name, ref2_designation: emp.ref2_designation,
    ref2_organization: emp.ref2_organization, ref2_address: emp.ref2_address,
    ref2_city_state_pin: emp.ref2_city_state_pin,
    ref2_contact_no: emp.ref2_contact_no, ref2_email: emp.ref2_email,
    ref3_name: emp.ref3_name, ref3_designation: emp.ref3_designation,
    ref3_organization: emp.ref3_organization, ref3_address: emp.ref3_address,
    ref3_city_state_pin: emp.ref3_city_state_pin,
    ref3_contact_no: emp.ref3_contact_no, ref3_email: emp.ref3_email,
    department: emp.department, position: emp.position,
    joining_date: emp.joining_date, employment_type: emp.employment_type,
    circle: emp.circle, project_name: emp.project_name,
    reporting_manager: emp.reporting_manager,
    bank_name: emp.bank_name, account_number: emp.account_number,
    ifsc_code: emp.ifsc_code, account_holder_name: emp.account_holder_name,
    bank_branch: emp.bank_branch,
    basic_salary: emp.basic_salary, hra: emp.hra,
    other_allowances: emp.other_allowances,
    address: emp.address, city: emp.city, state: emp.state,
    zip_code: emp.zip_code, status: emp.status, employee_id: emp.employee_id,
  };
}

/**
 * Restores an employee's fields from a snapshot and sets a new status.
 * Also clears rejoin_snapshot + active_rejoin_link_id.
 */
async function restoreFromSnapshot(client, empId, snapshot, newStatus = 'inactive') {
  ...
  await client.query(
    `UPDATE employees SET
       ...
       uan_number=$17,
       family_member_name=$18,          family_contact_no=$19,
       family_working_status=$20,       family_employer_name=$21,
       family_employer_contact=$22,
       emergency_contact_name=$23,      emergency_contact_no=$24,
       emergency_contact_address=$25,   emergency_contact_relation=$26,
       permanent_address=$27,           permanent_phone=$28,
       permanent_landmark=$29,          permanent_lat_long=$30,
       local_same_as_permanent=$31,     local_address=$32,
       local_phone=$33,                 local_landmark=$34,   local_lat_long=$35,
    ...
    [
      ...
      s.pan_number, s.name_on_pan, s.aadhar_number, s.name_on_aadhar,
      s.uan_number || null,
      ...
      s.ref1_name, s.ref1_designation, s.ref1_organization,
      s.ref1_address, s.ref1_city_state_pin, s.ref1_contact_no, s.ref1_email,
      s.ref2_name, s.ref2_designation, s.ref2_organization,
      s.ref2_address, s.ref2_city_state_pin, s.ref2_contact_no, s.ref2_email,
      s.ref3_name, s.ref3_designation, s.ref3_organization,
      s.ref3_address, s.ref3_city_state_pin, s.ref3_contact_no, s.ref3_email,
...
// ── Full SELECT (reused by getAll, getById, createEmployee, updateEmployee) ───
const EMP_SELECT = `
  SELECT
    e.id, e.employee_id,
    ...
    e.local_same_as_permanent, e.local_address, e.local_phone,
    e.local_landmark, e.local_lat_long,
    ...
    COALESCE(e.basic_salary, 0) + COALESCE(e.hra, 0) +
      COALESCE(e.other_allowances, 0)                  AS total_salary,
...
export async function getAll(req, res) {
  try {
    const { rows } = await pool.query(
      `${EMP_SELECT}
       WHERE e.status NOT IN ('pending','pending_rejoin')
       GROUP BY e.id
...
export async function cleanupExpiredRejoinInvites(req, res) {
  ...
  try {
    ...
    const { sendRejoinInviteExpiredEmail } = await import('../services/emailService.js')
      .catch(() => ({ sendRejoinInviteExpiredEmail: null }));
    ...
    for (const link of expiredLinks) {
      ...
      try {
        ...
        if (link.prefill_employee_id) {
          const { rows } = await client.query(
            `SELECT * FROM employees WHERE id=$1`, [link.prefill_employee_id]
          );
          emp = rows[0] || null;
        }
...
export async function exportData(req, res) {
  try {
    ...
    const workbook = new ExcelJS.Workbook();
    const ws = workbook.addWorksheet('Employees', { views: [{ state: 'frozen', ySplit: 1 }] });

    const headers = [
      'Employee ID','Status','First Name','Father/Husband','Last Name',
      'Email','Phone','Alt Phone','Date of Birth','Gender',
      ...
      'Circle','Project','Reporting Manager',
      'Basic Salary','HRA','Other Allowances','Total Salary',
...
export async function createEmployee(req, res) {
  ...
  try {
    await client.query('BEGIN');
    const b          = req.body;
    const uanNumber  = req.uanNumber; // set by validateUAN middleware
    ...
    `, [
      ...
      b.ref1Name?.trim() || null, b.ref1Designation?.trim() || null,
      b.ref1Organization?.trim() || null, b.ref1Address?.trim() || null,
      b.ref1CityStatePin?.trim() || null, b.ref1ContactNo?.trim() || null, b.ref1Email?.trim() || null,
      b.ref2Name?.trim() || null, b.ref2Designation?.trim() || null,
      b.ref2Organization?.trim() || null, b.ref2Address?.trim() || null,
      b.ref2CityStatePin?.trim() || null, b.ref2ContactNo?.trim() || null, b.ref2Email?.trim() || null,
      b.ref3Name?.trim() || null, b.ref3Designation?.trim() || null,
      b.ref3Organization?.trim() || null, b.ref3Address?.trim() || null,
      b.ref3CityStatePin?.trim() || null, b.ref3ContactNo?.trim() || null, b.ref3Email?.trim() || null,
      ...
      b.designation?.trim(), b.department, b.circle || null, b.projectName || null,
  ...
  try {
    ...
    const { rows } = await pool.query(`
      UPDATE employees SET
        ...
        emergency_contact_name=$23,    emergency_contact_no=$24,
        emergency_contact_address=$25, emergency_contact_relation=$26,
        permanent_address=$27,  permanent_phone=$28, permanent_landmark=$29,
        permanent_lat_long=$30,
      ...
      RETURNING *
    `, [
      b.firstName?.trim() || null, null, b.lastName?.trim() || null,
      b.fatherHusbandName?.trim() || null,
      b.email?.trim() || null, b.phone?.trim() || null, b.altPhone?.trim() || null,
      b.dob || null,
      b.gender || null, b.maritalStatus || null, b.educationalQualification || null,
      b.bloodGroup || null,
      b.panNumber?.trim() || null, b.nameOnPan?.trim() || null,
      b.aadhar?.replace(/\s/g, '') || null, b.nameOnAadhar?.trim() || null,
      uanNumber,
      ...
      b.ref1Name?.trim() || null, b.ref1Designation?.trim() || null,
      b.ref1Organization?.trim() || null, b.ref1Address?.trim() || null,
      b.ref1CityStatePin?.trim() || null, b.ref1ContactNo?.trim() || null,
      b.ref1Email?.trim() || null,
      b.ref2Name?.trim() || null, b.ref2Designation?.trim() || null,
      b.ref2Organization?.trim() || null, b.ref2Address?.trim() || null,
      b.ref2CityStatePin?.trim() || null, b.ref2ContactNo?.trim() || null,
      b.ref2Email?.trim() || null,
      b.ref3Name?.trim() || null, b.ref3Designation?.trim() || null,
      b.ref3Organization?.trim() || null, b.ref3Address?.trim() || null,
      b.ref3CityStatePin?.trim() || null, b.ref3ContactNo?.trim() || null,
      b.ref3Email?.trim() || null,
      b.department || null, b.designation?.trim() || null,
      b.employmentType || null, b.joiningDate || null,
      ...
      b.accountHolderName?.trim() || null, b.bankBranch?.trim() || null,
      String(id),
    ]);
...
export async function updateStatus(req, res) {
  ...
  try {
    ...
    const current = await client.query(
      `SELECT id, status, employee_id, department
...
export async function sendStatusNotification(req, res) {
  try {
    ...
    if (status === 'Active') {
      ...
      return res.json({ success: true, message: 'No notification email required for this status' });
    }

    return result.success
      ? res.json({ success: true, message: `Notification email sent to ${email}` })
...
export async function getHistory(req, res) {
  ...
  try {
    ...
    const countResult = await pool.query(
      `SELECT COUNT(*) AS cnt FROM employee_status_history WHERE employee_id=$1`, [id]
    );
    if (parseInt(countResult.rows[0].cnt, 10) === 0) {
      await pool.query(
        ...
        [
          id, employee.status,
          JSON.stringify({ employee_id: employee.employee_id, department: employee.department }),
...
export async function uploadPhoto(req, res) {
  ...
  try {
    ...
    const empResult = await client.query(
      `SELECT id FROM employees WHERE id::text=$1 OR employee_id=$1`, [String(id)]
    );
    if (empResult.rows.length === 0) {
    ...
    for (const doc of existing.rows) {
      try { fs.unlinkSync(path.join(PROJECT_ROOT, doc.file_path.replace(/^\//, ''))); } catch (_) {}
    ...
    await client.query('COMMIT');
    return res.json({ success: true, message: 'Photo uploaded successfully', data: rows[0] });
  } catch (err) {
    await client.query('ROLLBACK');
    try { fs.unlinkSync(req.file.path); } catch (_) {}
...
export async function sendRejoinInvite(req, res) {
  ...
  try {
    const { id } = req.params;
    const { rows } = await client.query(
      `SELECT * FROM employees WHERE id::text=$1 OR employee_id=$1`, [String(id)]
    );
    ...
    await sendRejoinInvitationEmail({
      ...
      employeeId: emp.employee_id, registrationUrl,
